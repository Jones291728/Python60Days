# 1.將索引(index)依序改為 sex、class、student_id，欄位依序改成chinese_score、english_score、math_score
import pandas as pd
...
print("分數原始資料:\n",score_df)                        
#方法_01
# print(score_df.pivot_table(index=["sex", "class", "student_id"]))
# #方法_02
newscore=pd.melt(score_df, id_vars =["sex", "class", "student_id"])
results=newscore.pivot(index=["sex", "class", "student_id"], columns='variable', values='value')
print("分數資料重新建構資料後的結果:\n",results)